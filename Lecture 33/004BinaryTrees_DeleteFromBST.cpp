...
TreeNode* deleteFromBST(TreeNode* root, int key) {
	...
	if (key < root->val) {
		...
		if (root->left == NULL and root->right == NULL) {
			...
			// node you want to delete is a node with single right child
			TreeNode* temp = root->right;
			delete root;
			root = temp;
		} else if (root->left != NULL and root->right == NULL) {
			// node you want to delete is a node with single left child
			TreeNode* temp = root->left;
			delete root;
			root = temp;
		} else {
			// node you want to delete is anode with two child nodes
