...
// time : O(n)
// space: O(n) due to queue, n/2 to be precise assuming tree is perfect then last level will have n/2 nodes
...
void printLevelOrder(TreeNode* root) {
	...
	while (!q.empty()) {
		...
		int qsize = q.size(); // qsize stores the size of the current level
