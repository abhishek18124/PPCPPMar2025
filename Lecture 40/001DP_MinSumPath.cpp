...
// time : O(mn)
// space: mn due to dp[][] + (m+n) due to function call stack ~ O(mn)
...
int fBottomUp(const vector<vector<int>>& grid, int m, int n) {
	...
	for (int i = m - 1; i >= 0; i--) {
		for (int j = n - 1; j >= 0; j--) {
			// dp[i][j] = f(i, j) = minSum path from i,jth to m-1,n-1th cell
			if (i == m - 1 and j == n - 1) {
				dp[i][j] = grid[i][j];
			} else if (i == m - 1) {
...
int fBottomUpSpaceOptmised(const vector<vector<int>>& grid, int m, int n) {
	vector<int> dp(n);
	for (int i = m - 1; i >= 0; i--) {
		for (int j = n - 1; j >= 0; j--) {
			// dp[i][j] = f(i, j) = minSum path from i,jth to m-1,n-1th cell
	...
	return dp[0]; // at the 0th index of dp[] stores dp[0][0] i.e. f(0, 0)
...
int main() {
	...
	cout << fBottomUpSpaceOptmised(grid, m, n) << endl;
